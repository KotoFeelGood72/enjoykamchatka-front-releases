<template>
    <div v-if="activity" class="activity-location">
        ...
        <dynamic v-if="activity.content" :template="`<div class='text--small activity-location__content content-block'><div class='container--middle-tour '>` + activity.content + `</div></div>`"/>
        <div class="container activity-location__bottom">
            <Heading v-if="activity.locations && activity.locations.length" class="location-title" tag="h2">Локации</Heading>
...
export default {
    ...
    data() {
        ...
        console.log(`activity/${params.activity}?router=${route.path}`)
        return Api.get(`activity/${params.activity}?router=${route.path}`).then((response) => {
